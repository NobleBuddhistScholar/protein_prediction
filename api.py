...
def train_model():
    """模型训练接口，支持上传文件夹（多个fasta），训练完成后返回模型和json"""
    ...
    try:
        ...
        logging.info(f'参数: model_type={model_type}, epochs={epochs}, batch_size={batch_size}, lr={learning_rate}, max_length={max_length}, threshold={threshold}')
        # 获取上传的文件夹（多个文件）
        files = request.files.getlist('file')
        logging.info(f'收到文件数: {len(files)}')
        if not files or len(files) == 0:
            logging.error('未上传任何数据文件')
            return jsonify({"error": "未上传任何数据文件"}), 400
        # 保存到临时目录
        temp_dir = tempfile.mkdtemp()
        for f in files:
            logging.info(f'处理文件: {f.filename}')
            # 只保留文件名，去除前端可能带的子目录
            save_path = os.path.join(temp_dir, os.path.basename(f.filename))
            f.save(save_path)
        logging.info(f'已保存所有上传文件到: {temp_dir}')
        config = {
        ...
        logging.error(f"train_model error: {str(e)}")
        return jsonify({"error": f"训练接口异常: {str(e)}"}), 500
